# Dark Null Protocol Historical Devnet Results

> Historical artifact bundle captured on January 11, 2026.
>
> This file is evidence about one historical devnet flow around program `33Uw9kiVRrn6wVmR439gA9QWh4MLv87N97taj2sLrkE4`.
>
> It is **not** proof that the current canonical root program, current root artifact set, or current root manifest are the same thing as this older historical bundle.

## Historical Scope Warning

- this file documents a historical test run, not a current release certification
- the current root program now has a real verifier path, but it is bound to `2stas3cZYnBiWpndcTXQDGLXwfQ7kjEYYrW52DsUAcxF`, not to the program ID in this file
- the current root circuit bundle is the canonical `null_proof` set under [`MANIFEST.json`](./MANIFEST.json), not the exact circuit story implied by this older bundle
- use [`docs/PROGRAM_IDS.md`](./docs/PROGRAM_IDS.md) before assuming any program ID in this repo is interchangeable with any other
- use [`historical/null-mint/MANIFEST.json`](./historical/null-mint/MANIFEST.json) if you want one recovered branch with program/artifact binding in-tree
...
| **Result** | Historical artifact claims all recorded phases passed |
...
## Captured API Status At The Time
...
3. Do not treat the proof-size, public-input, or circuit claims in this file as describing the current canonical root.
4. If you want the current root binding, use [`MANIFEST.json`](./MANIFEST.json).
5. If you want the recovered source provenance that fed the current root, use [`historical/null-mint/MANIFEST.json`](./historical/null-mint/MANIFEST.json).
...
*Historical devnet artifact captured on 2026-01-11*
*Not a current public-root cryptographic assurance statement*
