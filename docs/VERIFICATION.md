...
`prepare_phantom_withdraw` is the legacy three-input shape and remains fail-closed.

`prepare_phantom_withdraw_v2` is the promoted payout-bound public-input shape:

```text
[commitment, nullifier, root, amount, receiver_token_part_0, receiver_token_part_1, mint_part_0, mint_part_1]
```

It checks instruction arguments against that layout, verifies the promoted Groth16 proof, records the nullifier, and transfers from the vault token account to the receiver token account.

## Historical Proof-Pack Intent

Older proof-pack documents described recipient and amount binding as protocol intent. The current root now promotes that binding through the v2 circuit/artifact set, while historical documents remain non-canonical context.
